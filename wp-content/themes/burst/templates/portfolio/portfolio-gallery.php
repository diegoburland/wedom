...
$burst_mikado_columns_number 			= 'v4';
$burst_mikado_portfolio_title_tag            = 'h3';
$burst_mikado_portfolio_title_style          = '';
$burst_mikado_portfolio_gallery_hover_type 	= 'magnifier';
$burst_mikado_portfolio_gallery_image_hover_separator = '';
...
if (isset($burst_mikado_options['portfolio_gallery_image_hover_style'])) {
    $burst_mikado_portfolio_gallery_hover_type = $burst_mikado_options['portfolio_gallery_image_hover_style'];
}

//gallery hover separator with text
if (isset($burst_mikado_options['portfolio_gallery_image_hover_separator'])) {
    $burst_mikado_portfolio_gallery_image_hover_separator = $burst_mikado_options['portfolio_gallery_image_hover_separator'];
}
//is lightbox turned on for video single project?
if (isset($burst_mikado_options['lightbox_video_single_project'])) {
    $lightbox_video_single_project = $burst_mikado_options['lightbox_video_single_project'];
}
...
if (isset($burst_mikado_options['portfolio_info_tag'])) {
    $portfolio_info_tag = $burst_mikado_options['portfolio_info_tag'];
}
...
if(get_post_meta(get_the_ID(), "mkd_choose-number-of-portfolio-columns", true) !== "") {
	$burst_mikado_columns_number = 'v'.get_post_meta(get_the_ID(), "mkd_choose-number-of-portfolio-columns", true);
} elseif(isset($burst_mikado_options['portfolio_columns_number']) && $burst_mikado_options['portfolio_columns_number'] !== '') {
	$burst_mikado_columns_number = 'v'.$burst_mikado_options['portfolio_columns_number'];
}
    ...
    if ($portfolio_m_images){
        ...
        foreach($portfolio_image_gallery_array as $gimg_id){
            ...
            <?php if($burst_mikado_lightbox_single_project == "yes"){ ?>
                                <a class="lightbox_single_portfolio <?php echo esc_attr($burst_mikado_columns_number); ?>" title="<?php echo esc_attr($title); ?>" href="<?php echo esc_url($image_light_src); ?>" data-rel="prettyPhoto[single_pretty_photo]">
                   <?php if($burst_mikado_portfolio_gallery_hover_type !== "disable"){ ?>
                       <span class="gallery_text_holder"><span class="gallery_text_outer"><span class="gallery_text_inner">
                            <?php
                            if($burst_mikado_portfolio_gallery_hover_type == "icon"){ ?>
                                <i class="fa fa-plus"></i>
                            <?php }
                            elseif($burst_mikado_portfolio_gallery_hover_type == "text"){ ?>
                                <h4><?php echo esc_html($title); ?></h4>
                                <?php if($burst_mikado_portfolio_gallery_image_hover_separator == "yes"){ ?>
            ...
            <?php } else { ?>
                <a class="lightbox_single_portfolio <?php echo esc_attr($burst_mikado_columns_number); ?>" href="#">
    ...
    if (is_array($burst_mikado_portfolio_images) && count($burst_mikado_portfolio_images)){
		foreach($burst_mikado_portfolio_images as $portfolio_image){
			...
			<?php if($portfolio_image['portfolioimg'] != ""){ ?>
                ...
                $single_image_id = burst_mikado_get_attachment_id_from_url($portfolio_image['portfolioimg']);
				...
				?>
				<?php if($burst_mikado_lightbox_single_project == "yes"){ ?>
					<a class="lightbox_single_portfolio <?php echo esc_attr($burst_mikado_columns_number); ?>" title="<?php echo esc_attr($portfolio_image['portfoliotitle']); ?>" href="<?php echo esc_url($portfolio_image['portfolioimg']); ?>" data-rel="prettyPhoto[single_pretty_photo]">
                        <?php if($burst_mikado_portfolio_gallery_hover_type !== "disable"){ ?>
                            <span class="gallery_text_holder"><span class="gallery_text_outer"><span class="gallery_text_inner">
                            <?php
                            if($burst_mikado_portfolio_gallery_hover_type == "icon"){ ?>
				...
				switch ($portfolio_video_type){
					case "youtube": ?>
                        <?php if($lightbox_video_single_project == "yes"){ ?>
                            ...
                            ?>
                            <a class="lightbox_single_portfolio <?php echo esc_attr($burst_mikado_columns_number); ?>" title="<?php echo esc_attr($portfolio_image['portfoliotitle']); ?>" href="//www.youtube.com/watch?feature=player_embedded&v=<?php echo esc_attr($vidID); ?>" data-rel="prettyPhoto[single_pretty_photo]">
					...
					case "vimeo": ?>
                        <?php if($lightbox_video_single_project == "yes"){ ?>
							...
							WP_Filesystem();
                            $vidID = esc_attr($portfolio_image['portfoliovideoid']);
                            $xml = unserialize($burst_mikado_wp_filesystem->get_contents("https://vimeo.com/api/v2/video/$vidID.php"));
                            $thumbnail = $xml[0]['thumbnail_large'];
                            $video_title = $xml[0]['title'];
                            ?>
							<a class="lightbox_single_portfolio <?php echo esc_attr($burst_mikado_columns_number); ?>" title="<?php echo esc_attr($portfolio_image['portfoliotitle']); ?>" href="//vimeo.com/<?php echo esc_attr($portfolio_image['portfoliovideoid']); ?>" data-rel="prettyPhoto[single_pretty_photo]">
                                <i class="fa fa-play"></i>
                                <img width="100%" src="<?php echo esc_url($thumbnail); ?>"/>
							</a>
						<?php } else { ?>
					...
					case "self": ?>
						...
						<a class="lightbox_single_portfolio <?php echo esc_attr($burst_mikado_columns_number); ?>" onclick='return false;' href="#">
							<div class="video">
