...
$args = array_merge($args, $burst_mikado_IconCollections->getShortcodeParams('content_menu_'));
...
$css_class_overflow = "";
if ($overflow != ''){
	$css_class_overflow = " row_overflow_".$overflow;
}
...
if ($icon_pack !== '' && $in_content_menu !== '') {
    ...
    $item_collection_icon_obj = $burst_mikado_IconCollections->getIconCollection($icon_pack);
...
if($row_type == 'row') {
	$output .= '<div '.$row_id.' '.$anchor_id.' '.$menu_title.' '.$menu_icon.' '.$header_style_data.' class="' . $css_class . $css_class_type . $css_class_in_content_menu . $css_class_video . $use_row_as_box_class . $css_class_pattern . $full_screen_section_class . $css_class_overflow . '"';
	...
	if($video == "show_video"){
        ...
        if($use_row_as_full_screen_section == "yes"){
            ...
            $output .='<object width="320" height="240" type="application/x-shockwave-flash" data="'.get_template_directory_uri().'/js/flashmediaelement.swf">
										...
										<img src="'.$v_image.'" width="1920" height="800" title="' . esc_attr__( 'No video playback capabilities', 'burst' ) . '" alt="' . esc_attr__( 'Video thumb', 'burst' ) . '" />
								</object>
						</video>';
        } else {
    ...
    $zoom_effect_class = '';
    if($zoom_effect == 'yes'){
        $zoom_effect_class = 'zoom_effect';
    }

    $output .='<section '.$row_id.' '.$anchor_id.' '.$menu_title.' '.$menu_icon.' '.$header_style_data.' data-speed="'. $parallax_speed .'" class="parallax_section_holder '.$css_class_in_content_menu.' ' . $css_class_type_parallax . ' '.$full_screen_section_height_class.' '.$vertically_align_content_in_middle_class.'" style = "';
    ...
    $output .= ($background_image !== '' || $background_image !== ' ') ? '<div class="parallax_section_holder_background '.$css_class_preload_background.' '.$zoom_effect_class.'" style="background-image:url(' . $_image[0] . ');"></div>' : '';
	...
	if($logo_in_content_menu == 'logo_in_content_menu'){
		$output .= '<div class="logo">';
			...
			if(isset($burst_mikado_options['content_menu_logo_image']) && $burst_mikado_options['content_menu_logo_image'] != ""){
				$logo_image = $burst_mikado_options['content_menu_logo_image'];
			}else if(isset($burst_mikado_options['logo_image']) && $burst_mikado_options['logo_image'] != ""){
				$logo_image = $burst_mikado_options['logo_image'];
			}
			$output .= '<a href="'.esc_url(home_url('/')).'"><img src="'.esc_url($logo_image).'" alt="' . esc_attr__( 'Logo', 'burst' ) . '"/></a>';
		$output .= '</div>';
	}
	...
	if($custom_widget_area !== ''){
		$output .= '<div class="custom_widget_area"><div class="custom_widget_area_inner"><div class="custom_widget_area_inner2">';
			$output .= burst_mikado_get_dynamic_sidebar($custom_widget_area);
		$output .= '</div></div></div>';
	}
...
echo burst_mikado_get_module_part($output);