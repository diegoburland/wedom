...
$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'wpb_accordion_section group', $this->settings['base']);
	...
	$output .= '<span class="accordion_mark left_mark"><span class="accordion_mark_icon" '.burst_mikado_get_inline_style($icon_style).'><span class="icon_plus"></span><span class="icon_minus-06"></span></span></span>';
    ...
    $output .= "\n\t\t\t\t" . '<div ' . ( isset( $el_id ) && ! empty( $el_id ) ? "id='" . esc_attr( $el_id ) . "'" : "" ) . ' class="accordion_content">';
		$output .= "\n\t\t\t" . '<div class="accordion_content_inner" '.burst_mikado_get_inline_style($content_style).'>';
