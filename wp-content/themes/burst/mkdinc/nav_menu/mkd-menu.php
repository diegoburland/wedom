...
if (!function_exists('burst_mikado_add_custom_nav_fields')) {
	function burst_mikado_add_custom_nav_fields($menu_item) {
	...
	add_filter( 'wp_setup_nav_menu_item', 'burst_mikado_add_custom_nav_fields' );

}

if (!function_exists('burst_mikado_update_custom_nav_fields')) {
	/**
	 ...
	 * @return      void
	 */
	function burst_mikado_update_custom_nav_fields($menu_id, $menu_item_db_id, $args) {
	...
	// save menu custom fields
	add_action( 'wp_update_nav_menu_item', 'burst_mikado_update_custom_nav_fields', 10, 3 );

}

if (!function_exists('burst_mikado_edit_walker')) {
	/**
	 ...
	 * @return      void
	 */
	function burst_mikado_edit_walker($walker, $menu_id) {

		return 'BurstMikadoWalkerNavMenuEditCustom';

	}


// edit menu walker
	add_filter( 'wp_edit_nav_menu_walker', 'burst_mikado_edit_walker', 10, 2 );
...
if (!class_exists('BurstMikadoType1WalkerNavMenu')) {
	class BurstMikadoType1WalkerNavMenu extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$wide_menu_background = isset($burst_mikado_options['enable_wide_manu_background']) && $burst_mikado_options['enable_wide_manu_background'] == 'yes';
			
			$indent = str_repeat("\t", $depth);
			if($depth == 0){
				$out_div = '<div class="second'.$background_image_class.'" '.($wide_menu_background ? burst_mikado_get_inline_style($menu_image_background) : '').'><div class="inner">';
			}else{
				$out_div = '';
			}

			// build html
			$output .= "\n" . $indent . $out_div  .'<ul '.(!$wide_menu_background ? burst_mikado_get_inline_style($menu_image_background) : '').'>' . "\n";
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$wide_background = '';
			if(isset($burst_mikado_options['enable_wide_manu_background']) && $burst_mikado_options['enable_wide_manu_background'] == "yes" ){
			...
			if($item->hide == ""){
                ...
                if($depth == 0 && isset($burst_mikado_options['enable_dropdown_menu_item_icon']) && $burst_mikado_options['enable_dropdown_menu_item_icon'] == "yes" ){
				...
				if(isset($burst_mikado_options['menu_item_hover_animation']) && $burst_mikado_options['menu_item_hover_animation'] == "roll" && $depth == 0){
...
if (!class_exists('BurstMikadoType2WalkerNavMenu')) {
	class BurstMikadoType2WalkerNavMenu extends Walker_Nav_Menu {
...
if (!class_exists('BurstMikadoType3WalkerNavMenu')) {
    class BurstMikadoType3WalkerNavMenu extends Walker_Nav_Menu {
...
if (!class_exists('BurstMikadoType4WalkerNavMenu')) {
    class BurstMikadoType4WalkerNavMenu extends Walker_Nav_Menu {
